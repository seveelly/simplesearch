  ...
  class Autocomplete {
    constructor(_element, _ref) {
      ...
      this._reset = () => {
        ...
        if (
          (((_this$_matches = this._matches) == null
            ? void 0
            : _this$_matches.length) == 0 &&
            !this._toInput) ||
          this._showAll
        ) {
      ...
      this._onLoading = (type) =>
        this._root.parentNode.classList[type ? "add" : "remove"](
          this._isLoading
        );
      ...
      this._handleMouse = (event) => {
        ...
        const targetClosestRole =
          targetClosest == null ? void 0 : targetClosest.hasAttribute("role");
      ...
      this._setAria = (target) => {
        const selectedOption =
          this._selectedOption + "-" + this._indexLiSelected(target);
      ...
      this._onSearch = isPromise(onSearch)
        ? onSearch
        : (_ref4) => {
            let { currentValue, element } = _ref4;
